...
Shamelessly inspired/ripped-off from
[misterio77](https://github.com/misterio77/nix-config) and
[archer-65](https://github.com/archer-65/nix-dotfiles)

## Structure

- `flake.nix`: Entrypoint for hosts and home configurations. Also exposes
  devshells for boostrapping (`nix develop` or `nix-shell`).
...
8. Enable secure boot using directions from
   [nix-community/lanzaboote](https://github.com/nix-community/lanzaboote),
   merging in `lanzaboote` from the existing git
